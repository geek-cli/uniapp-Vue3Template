<template>
	<view>
		<payment-password-modal ref="ssModal" :mode="_parentMode" :position="_parentPosition" @change="changeModal">
...
<script setup>
	 ...
	 */
	import paymentPasswordModal from './payment-password-modal.vue';
	...
	let getKeyNumber = (val)=>{
		...
		if (val === '' || (val != -1 && paymentPwdArr.length === _digits.value)) { //空或者已经达到最大值
			...
			if (activeInput.value >= 0) {
				...
				paymentPwd.value = paymentPwdArr.join('');
				paymentPwds[current] = paymentPwd.value;
			}
		}
	}
	let getKeyboard = e=>{
